...
def post_review():
    ...
    user_id = get_jwt_identity()  # ✅ Obtener el usuario autenticado desde el token
    
    stars = data.get("stars", 0)
    descripcion = data.get("description", "").strip()

    # ✅ Obtener el nombre y el DNI del usuario desde la base de datos
    usuario = obtener_usuario_desde_bd(user_id)

    if not usuario or not usuario.get("dni"):
        return jsonify({"error": "No se encontró el usuario o el DNI"}), 400  # ❌ Evita enviar si no hay datos válidos
    ...
    # ✅ Enviar la reseña a la API Mock
